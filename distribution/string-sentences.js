...
var abbrvNoEOS = Object.create(null);
abbrvNoEOS['mr.'] = true;
abbrvNoEOS['mrs.'] = true;
abbrvNoEOS['ms.'] = true;
abbrvNoEOS['er.'] = true;
...
var abbrvMayBeEOS = Object.create(null);
 ...
 */
var punkt = function punkt(paragraph) {
  ...
  var paraTokens = paragraph.split(' ');
  var sentenceTokens = [];
  var sentences = [];

  for (var k = 0; k < paraTokens.length; k += 1) {
    // A para token.
    var pt = paraTokens[k];
    // A lower cased para token.
    var lcpt = pt.toLowerCase();
    if (rgxPotentialEOS.test(pt) && !abbrvNoEOS[lcpt] && (pt.length !== 2 || rgxAlphaAt0.test(pt) || rgxSplI.test(lcpt))) {
      ...
      if (abbrvMayBeEOS[lcpt]) {
        for (var j = k + 1; j < paraTokens.length && !nextpt; j += 1) {
          nextpt = paraTokens[j];
        }
      }
